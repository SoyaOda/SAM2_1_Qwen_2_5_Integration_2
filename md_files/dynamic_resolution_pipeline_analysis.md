# LISA改 動的解像度パイプライン完全解析
## 2025年8月8日 作成

# 1. 概要

LISA改は、Qwen2.5-VL-3BとSAM2.1を統合した次世代のマルチモーダル理解モデルです。本文書では、動的解像度（Dynamic Resolution）環境下での画像処理パイプライン全体を詳細に解析し、元画像からLoss計算までの完全なデータフローを記録します。
...
```
元画像 (PIL.Image)
  ├─[Qwen処理]─→ pixel_values [N_raw, 1176]
  │                ├─[PatchMerge]─→ [N_merge, 2048]
  │                └─[Adapter]─→ [256, H_feat, W_feat]
  │
  └─[SAM処理]──→ sam_images [3, 1024, 1024]
                   └─[MaskDecoder]─→ masks [1, H_orig, W_orig]
```
...
|---------|------|-------------------|
| 元画像 | 可変 | 各サンプル異なる |
| Collator後pixel_values | [B, N_max, 1176] | [4, 900, 1176] |
| image_grid_thw | [B, 3] | [4, 3]（全て[1,30,30]） |
| vision_features | [B, N_merge, 2048] | [4, 225, 2048] |
| image_features_sam | [B, 256, H, W] | [4, 256, 15, 15] |
| 予測マスク | [B, 1, H_orig, W_orig] | 各サンプル異なる |
