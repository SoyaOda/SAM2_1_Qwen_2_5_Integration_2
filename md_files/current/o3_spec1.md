了解しました。Qwen2.5-VL-3BとSAM2.1をPyTorch + HuggingFaceベースで統合し、将来的なFoodLMM改に耐え得る設計を目指して、トークナイザ拡張、統合方法、LoRA設計、形状整合を含む明確かつ曖昧性のないモデル設計と実装スクリプトを徹底的にリサーチ・構築します。

SAM2.1とQwen2.5-VLの構造解析、重み付き読み込み方法、統合におけるtensor整合性、形状対応プロンプト生成の仕組みをすべて動作可能なコードで提供します。詳細な実装はモジュール単位で分離し、コードの可読性と再利用性を確保します。日本語での出力で進行します。
