#!/usr/bin/env python3
"""
Test A-4: 加算アプローチ（位置＋言語）によるseg loss改善テスト

解決策B: 位置情報を100%保持しつつ、LLM埋め込みを加算で組み合わせる。
残差接続的なアプローチで、位置情報を完全に保ちながらLLMの情報を加える。

期待される結果:
- seg lossが安定して減少する
- 位置情報を破壊せずにLLM埋め込みの情報を取り込む
- 学習可能なスケーリング係数βで最適な混合比を自動調整
"""
...
# 加算アプローチのパラメータ（test_a4固有）
initial_beta = 0.01  # 初期のスケーリング係数（小さく開始）


def save_visualization(output_dir, step, batch, pred_mask, gt_mask, losses, tokenizer, beta):
    """可視化結果を保存（test_a1スタイル、beta値も表示）"""
    ...
    color = 'green' if beta < 0.5 else 'blue'
    ax3.set_title(f'Predicted Mask (β={beta:.3f})', fontsize=12, fontweight='bold', color=color)
    ax3.axis('off')
    ...
    info_text += f"β (LLM scaling factor): {beta:.3f}\n\n"
    info_text += f"Losses:\n"
    ...
    # βの解釈
    if beta < 0.1:
        info_text += "Mode: Minimal LLM contribution\n"
    elif beta < 0.3:
        info_text += "Mode: Light LLM addition\n"
    elif beta < 0.5:
        info_text += "Mode: Moderate LLM addition\n"
    else:
        info_text += "Mode: Strong LLM addition\n"
    
    ax6.text(0.05, 0.95, info_text, transform=ax6.transAxes,
            fontsize=10, verticalalignment='top',
            bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5),
            family='monospace')
    ax6.set_title('Addition Information', fontsize=12, fontweight='bold')
    
    plt.suptitle(f'Addition Embedding Test - Step {step} (β={beta:.3f})', fontsize=14, fontweight='bold')
    plt.tight_layout()
    ...
    logger.info(f"Saved visualization to {save_path} (Dice: {dice:.4f}, IoU: {iou:.4f}, β: {beta:.3f})")
...
def main():
    ...
    output_dir = create_standard_output_structure(f"test_outputs/test_a4_{timestamp}")
    logger.info(f"Output directory: {output_dir}")
    
    # テスト設定を保存（最初は基本設定のみ）
    save_test_config(output_dir, 'test_a4_addition_embedding', None, {
        'test_description': 'Addition approach preserving 100% position information',
    ...
    config = LISAConfig(
        ...
        # Freeze configuration - Test A4: Addition Embedding
        ...
        freeze_prompt_beta=False        # A4: Addition embedding用Beta学習可能
    )
    ...
    save_test_config(output_dir, 'test_a4_addition_embedding', config, {
        'test_description': 'Addition approach preserving 100% position information',
        'initial_beta': initial_beta,
    ...
    with tqdm(total=num_steps, desc="Training with Addition Embedding") as pbar:
        while step < num_steps:
            ...
            # **修正後: SAM画像を含むモデルforward（加算アプローチ適用）**
            ...
            beta_history.append(beta_value)
            ...
            if len(pred_masks_for_vis) > 0 and len(gt_masks_for_vis) > 0:
                ...
                save_visualization(output_dir, step, batch, pred_masks_for_vis[0], 
                                 gt_masks_for_vis[0], losses_dict, tokenizer, beta_value)
            ...
            if step % LOG_INTERVAL == 0:
                logger.info(f"Step {step}: total={total_loss.item():.4f}, "
                          f"lm={lm_loss.item():.4f}, seg={seg_loss.item():.4f}, β={beta_value:.3f}")
    ...
    try:
        ...
        # Beta evolution
        axes[1, 1].plot(steps, beta_history, color='blue')
        axes[1, 1].set_xlabel('Step')
        axes[1, 1].set_ylabel('Beta (LLM scaling factor)')
        axes[1, 1].set_title('Beta Evolution')
        axes[1, 1].grid(True)
        axes[1, 1].axhline(y=initial_beta, color='r', linestyle='--', label=f'Initial β={initial_beta}')
        axes[1, 1].axhline(y=beta_history[-1], color='g', linestyle='--', label=f'Final β={beta_history[-1]:.3f}')
        axes[1, 1].legend()
    ...
    if final_seg_loss < initial_seg_loss * EVAL_THRESHOLD:
        logger.info("✅ TEST PASSED: Segmentation loss decreased significantly with addition embedding")
        ...
        logger.info(f"   Final beta: {beta_history[-1]:.3f}")
    else:
        ...
        logger.warning(f"   Beta evolved from {initial_beta:.3f} to {beta_history[-1]:.3f}")
        logger.warning("   Addition embedding may need parameter tuning")
