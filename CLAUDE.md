...
〇本タスク：minimal_train.pyを進めた場合のSEG LOSSがほぼ下がらない問題がある。そこでGTP-5proによりTestTest方針をmd_files/current/Focus_Seg_Loss20250810_spec.mdに作成したので、順にTest行うこと。まずA-1のテストスクリプトを作成してテストして。O3 queryも併用しながらタスク別に適切な前処理を行い、実装を行うこと。

※モデルやデータははminimal_train.pyで呼び出している統合モデル（lisa_model）やデータ(Hybriddataset)を指すと思われるので、異なる構造のモデルやデータを用いないよう注意すること
