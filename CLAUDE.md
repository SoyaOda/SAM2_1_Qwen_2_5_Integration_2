...
[命令]
〇事前タスク：minimal_train.pyとその関連Scriptをすべてよく読んで（長い場合は分割しながら読んで）現状の実装を理解して

〇本タスク：minimal_train.pyを進めた場合のSEG LOSSがほぼ下がらない問題がある。そこでGTP-5proにより修正方針をmd_files/current/Focus_Seg_Loss20250809_spec.mdに作成した。これを元に修正していきたい。問題1は実装完了した（実装レコードはmd_files/Focus_Seg_Loss20250809_record.md）。次に問題2: マルチマスク出力の複雑さと修正策の実装を行って。実際の生の元データセットの形状をよく把握して、O3 queryも併用しながらタスク別に適切な前処理を行い、実装を行うこと。
...
・Focus_Seg_Loss20250809_spec.mdはあくまでおおまかな指針であるので、細かな実装はWebでベストプラクティスをリサーチして[Introduction]に述べている目的に沿うように、本質的に実装を進めること（簡易な実装でとりあえず走るコードは必要ない、本質的に目標を達成するコードが欲しい）
