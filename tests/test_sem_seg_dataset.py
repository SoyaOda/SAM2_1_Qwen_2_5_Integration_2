...
def test_sem_seg_dataset():
    """セマンティックセグメンテーションデータセットの単体テスト"""
    print("=== セマンティックセグメンテーションデータセットテスト ===\n")
    ...
    try:
        sem_seg_dataset = SemSegDataset(
            ...
            exclude_val=False,
            sem_seg_data="ade20k"  # ADE20Kのみでテスト
        )
        print(f"   ✓ データセット初期化成功")
        print(f"   ✓ 有効なデータセット: {sem_seg_dataset.sem_seg_datas}")
    except Exception as e:
        print(f"   ✗ 初期化エラー: {e}")
        import traceback
        traceback.print_exc()
